...
const DEV_PATRON_LEVEL = 10
const DEV_PATRON_OPTION = 'dev-patron'

function isDevBuild() {
    const m = browser.runtime.getManifest()
    return /\[DEV\]/i.test(m.short_name || m.name || '')
}

async function isDevPatronEnabled() {
    if (!isDevBuild()) {
        return false
    }
    const options = (await sauce.storage.get('options')) || {}
    return !!options[DEV_PATRON_OPTION]
}

async function applyDevPatron() {
    if (!(await isDevPatronEnabled())) {
        return false
    }
    await sauce.storage.remove('patreon-auth')
    await _setPatronCache(DEV_PATRON_LEVEL, false)
    console.info(
        'Sauce dev patron enabled for testing (level %s)',
        DEV_PATRON_LEVEL,
    )
    return true
}
...
async function updatePatronLevel(athleteId) {
    if (await isDevPatronEnabled()) {
        return await _setPatronCache(DEV_PATRON_LEVEL, false)
    }
    let legacy = false
    let level = 0
    const errors = []
...
async function _setPatronCache(level, isLegacy) {
    await sauce.storage.set({
        ...
        patronLevelExpiration:
            Date.now() + (level ? 7 * 86400 * 1000 : 3600 * 1000),
    })
...
export async function syncDevPatronFromOptions() {
    if (await applyDevPatron()) {
        return
    }
    if (!isDevBuild()) {
        return
    ...
    const options = (await sauce.storage.get('options')) || {}
    if (!options[DEV_PATRON_OPTION]) {
        const athleteId = await sauce.storage.get('currentUser')
        await updatePatronLevel(athleteId)
    }
}
